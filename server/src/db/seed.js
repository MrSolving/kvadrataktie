...
export function runSeed() {
  ...
  insertPeriod.run({ id: uuid(), name: 'Handelsperiod Q1 2026', start_date: '2026-01-15', end_date: '2026-01-31', price: 145.0, lot_size: 100, status: 'closed' });

  const openPeriodId = uuid();
  insertPeriod.run({ id: openPeriodId, name: 'Handelsperiod Q3 2026', start_date: '2026-06-10', end_date: '2026-06-30', price: 150.0, lot_size: 100, status: 'open' });

  insertPeriod.run({ id: uuid(), name: 'Handelsperiod Q4 2026', start_date: '2026-10-01', end_date: '2026-10-15', price: null, lot_size: 100, status: 'upcoming' });
  ...
  `);
  insertOrder.run({ id: uuid(), user_id: ownerIds['anna@example.se'],   trading_period_id: openPeriodId, type: 'sell', quantity: 200, remaining_quantity: 200, status: 'initierad' });
  insertOrder.run({ id: uuid(), user_id: ownerIds['bertil@example.se'], trading_period_id: openPeriodId, type: 'buy',  quantity: 100, remaining_quantity: 100, status: 'initierad' });

  db.prepare(`INSERT INTO news (id, title, body, author_id) VALUES (?, ?, ?, ?)`).run(
    uuid(),
    'Välkommen till Kvadrat Aktiehandel',
    'Den nya portalen för intern aktiehandel är nu öppen. Här kan du se ditt innehav, lägga order under handelsperioder och följa dina affärer.',
    adminId
  );
  ...
  insertSession.run({ id: kongressId, title: 'Kongress 2026', type: 'kongress', description: 'Ordinarie kongress för Kvadratare. Varje aktieägare har en röst oavsett aktieinnehav.', status: 'open', created_by: adminId });
  insertMotion.run({ id: uuid(), session_id: kongressId, title: 'Ska Kvadrat utreda om dokusåpastjärnor ska kunna bli Kvadratare för deras gig på events mm?', description: 'Motionen föreslår att styrelsen tillsätter en utredning kring huruvida Kvadrat bör öppna upp för att anlita profiler från dokusåpor som konsulter vid kunduppdrag och events. Utredningen ska redovisas på nästa kongress.', sort_order: 1 });

  const stammaId = uuid();
  insertSession.run({ id: stammaId, title: 'Bolagsstämma 2026', type: 'stamma', description: 'Extra bolagsstämma. Rösträtten beräknas per aktie — privata och företagsinnehav räknas samman.', status: 'open', created_by: adminId });
  insertMotion.run({ id: uuid(), session_id: stammaId, title: 'Ska Kvadrat investera 10 MSEK för att starta Kvadrat New York och ändra koncernspråk till engelska?', description: 'Styrelsen föreslår att bolaget investerar 10 miljoner kronor i etableringen av ett New York-kontor samt att engelska införs som koncernspråk i all intern kommunikation och dokumentation. Investeringen finansieras via befintliga likvida medel.', sort_order: 1 });

  const insertCal = db.prepare(`INSERT INTO trading_calendar (id, label, start_date, end_date, notes) VALUES (@id, @label, @start_date, @end_date, @notes)`);
